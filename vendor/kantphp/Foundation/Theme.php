...
class Theme extends Component
{
    ...
    {
        $this->_baseUrl = rtrim(Kant::getAlias($url), '/');
    }
    ...
    {
        $this->_basePath = Kant::getAlias($path);
    }
    ...
    public function applyTo($path)
    {
        ...
        if (empty($pathMap)) {
            ...
            $pathMap = [Kant::$app->getBasePath() => [$basePath]];
        }

        $path = FileHelper::normalizePath($path);

        foreach ($pathMap as $from => $tos) {
            $from = FileHelper::normalizePath(Kant::getAlias($from)) . DIRECTORY_SEPARATOR;
            if (strpos($path, $from) === 0) {
                $n = strlen($from);
                foreach ((array) $tos as $to) {
                    $to = FileHelper::normalizePath(Kant::getAlias($to)) . DIRECTORY_SEPARATOR;
