...
class BaseUrl {
    ...
    public function route($name, $parameters = [], $absolute = true) {
        if (!is_null($route = Kant::$app->getRouter()->routes->getByName($name))) {
...
//        $url = Kant::getAlias($url);
        if ($url === '') {
            $url = Kant::$app->getRequest()->getUrl();
        }
        ...
        if (($pos = strpos($url, ':')) === false || !ctype_alpha(substr($url, 0, $pos))) {
            ...
            $url = $this->getRequest()->root() . '/' . ltrim($url, '/');
        }
    ...
    public static function remember($url = '', $name = null) {
        ...
        if ($name === null) {
            Kant::$app->getUser()->setReturnUrl($url);
        } else {
            Kant::$app->getSession()->set($name, $url);
        }
    }
    ...
    public static function previous($name = null) {
        if ($name === null) {
            return Kant::$app->getUser()->getReturnUrl();
        } else {
            return Kant::$app->getSession()->get($name);
        }
    }
    ...
    public static function canonical() {
        $params = Kant::$app->controller->actionParams;
        $params[0] = Kant::$app->controller->getRoute();
    ...
    public static function home($scheme = false) {
        $url = static::getHomeUrl();
        if ($scheme) {
            $url = $this->getRequest()->root() . $url;
            if (is_string($scheme) && ($pos = strpos($url, '://')) !== false) {
                $url = $scheme . substr($url, $pos);
            }
        }
    ...
    public static function current(array $params = [], $scheme = false) {
        $currentParams = Kant::$app->getRequest()->getQueryString();
        $currentParams[0] = '/' . Kant::$app->getRouter()->getRoutes();
     ...
     * @return string
     */
    protected static function formatScheme($secure = "") {
        if (!is_null($secure)) {
    ...
    protected static function getUrlManager() {
        return static::$urlManager ?: Kant::$app->getUrlManager();
    }

    protected function getRequest() {
        return Kant::$app->getRequest();
    }
    ...
    protected function getHomeUrl() {
        if ($this->getRequest()->showScriptName) {
