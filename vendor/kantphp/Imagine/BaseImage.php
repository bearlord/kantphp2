...
class BaseImage
{
    ...
    protected static function ensureImageInterfaceInstance($image)
    {
        ...
        if (is_string($image)) {
            return static::getImagine()->open(Kant::getAlias($image));
        }
    ...
    public static function text($image, $text, $fontFile, array $start = [0, 0], array $fontOptions = [])
    {
        ...
        $img = self::ensureImageInterfaceInstance($image);
        $font = static::getImagine()->font(Kant::getAlias($fontFile), $fontSize, $color);
    ...
    {
        $img = static::getImagine()->open(Kant::getAlias($image));
