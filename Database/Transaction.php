...
class Transaction extends Object
{
    ...
    public function begin($isolationLevel = null)
    {
        ...
        if ($this->_level === 0) {
            ...
            Kant::trace('Begin transaction' . ($isolationLevel ? ' with isolation level ' . $isolationLevel : ''), __METHOD__);
    ...
    public function commit()
    {
        ...
        $this->_level --;
        if ($this->_level === 0) {
            Kant::trace('Commit transaction', __METHOD__);
            $this->db->pdo->commit();
        ...
        $schema = $this->db->getSchema();
        if ($schema->supportsSavepoint()) {
            Kant::trace('Release savepoint ' . $this->_level, __METHOD__);
            $schema->releaseSavepoint('LEVEL' . $this->_level);
        } else {
            Kant::info('Transaction not committed: nested transaction not supported', __METHOD__);
        }
    }
    ...
    public function rollBack()
    {
        ...
        $this->_level --;
        if ($this->_level === 0) {
            Kant::trace('Roll back transaction', __METHOD__);
        ...
        $schema = $this->db->getSchema();
        if ($schema->supportsSavepoint()) {
            Kant::trace('Roll back to savepoint ' . $this->_level, __METHOD__);
            $schema->rollBackSavepoint('LEVEL' . $this->_level);
        } else {
            Kant::info('Transaction not rolled back: nested transaction not supported', __METHOD__);
