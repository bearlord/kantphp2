 ...
 * return [
 * [
 * 'class' => 'Kant\Filter\HttpCache',
 * 'only' => ['index'],
 * 'lastModified' => function ($action, $params) {
 * $q = new \Kant\Database\Query();
...
class HttpCache extends ActionFilter
{
    ...
    protected function validateCache($lastModified, $etag)
    {
        if (isset($_SERVER['HTTP_IF_NONE_MATCH'])) {
            ...
            return $etag !== null && in_array($etag, Kant::$app->request->getETags(), true);
    ...
    protected function sendCacheControlHeader()
    {
        if ($this->sessionCacheLimiter !== null) {
            if ($this->sessionCacheLimiter === '' && ! headers_sent() && Kant::$app->getSession()->getIsActive()) {
        ...
        $headers = Kant::$app->getResponse()->getHeaders();
