 ...
 * return [
 * 'pageCache' => [
 * 'class' => 'Kant\Filter\PageCache',
 * 'only' => ['index'],
 * 'duration' => 60,
 * 'dependency' => [
 * 'class' => 'Kant\Caching\DbDependency',
...
class PageCache extends ActionFilter
{
     ...
     *     
     *      If [[cacheCookies]] or [[cacheHeaders]] is enabled, then [[\Kant\Caching\Dependency::reusable]] should be enabled as well to save performance.
    ...
    /**
     *
     * @var \Kant\View\View the view component to use for caching. If not set, the default application view component
     *      [[\Kant\Application::view]] will be used.
     */
    public $view;
    ...
    public function beforeAction($action)
    {
        ...
        if (is_array($this->dependency)) {
            $this->dependency = Kant::createObject($this->dependency);
        }
        ...
        if (! is_array($data) || ! isset($data['cacheVersion']) || $data['cacheVersion'] !== 1) {
            ...
            Kant::trace('Valid page content is not found in the cache.', __METHOD__);
            return true;
        } else {
            $this->restoreResponse($response, $data);
            Kant::trace('Valid page content is found in the cache.', __METHOD__);
            return false;
        }
    }
